...
(function () {
  ...
  function abrirPerfil() {
    ...
    abrirGaveta(`
      ...
      <p class="muted">${esc(u.email)} · ${esc(S.nomeNivel(u.nivel))}${u.departamento ? ' · ' + esc(u.departamento) : ''}${FIN(u) ? ' · somente consulta' : ' · alçada ' + brl(S.limiteDoUsuario(u))}</p>
  ...
  function vInicio(el, u) {
    ...
    el.innerHTML = `
      ...
      <div class="kpis">
        <div class="kpi destaque"><div class="lbl">Sua alçada</div><div class="val">${brl(S.limiteDoUsuario(u))}</div><div class="sub">${nv(u) >= 3 && u.departamento ? 'Teto do Gerente · ' + esc(u.departamento) : 'Aprova sozinho até este valor'}</div></div>
  ...
  function tabela(lista, extra) {
    ...
    return `<div class="table-wrap"><table>
      <thead><tr><th>Nº</th><th>Data</th><th class="hide-sm">Solicitante</th><th class="hide-sm hide-md">Modalidade</th><th class="hide-sm hide-md">Centro de custo</th><th class="r">Total</th><th>Status</th>${extra ? '<th></th>' : ''}</tr></thead>
  ...
  const NOME_LISTA = { departamento: 'Departamento', modalidade: 'Modalidade', filial: 'Filial (centro de custo)', categoria: 'Categoria de itens', unidade: 'Unidade' };
  ...
  function vNova(el, u) {
    ...
    el.innerHTML = `
      ...
      <form id="fNova">
        ...
        <fieldset class="card fs-card" id="cardProd">
          <div class="card-head"><h2 class="card-titulo" style="margin:0"><span class="passo">3</span> <span id="titProd">Itens</span></h2><button type="button" class="btn btn-ghost btn-sm" id="addItem">${icon('plus', 16)} Adicionar item</button></div>
        ...
        <fieldset class="card fs-card" id="cardServ" hidden disabled>
          <div class="card-head"><h2 class="card-titulo" style="margin:0"><span class="passo">3</span> Serviços</h2><button type="button" class="btn btn-ghost btn-sm" id="addServ">${icon('plus', 16)} Adicionar serviço</button></div>
          <div class="table-wrap"><table class="itens-table">
            <thead><tr><th style="width:44%">Descrição</th><th>Valor (R$)</th><th>Prazo</th><th class="r">Subtotal</th><th></th></tr></thead>
    ...
    // --- itens ---
    const body = el.querySelector('#itensBody'), servBody = el.querySelector('#servBody');
    const novoServ = () => ({ descricao: '', valorUnit: '', prazo: '' });
    const servs = [novoServ()];
    ...
    const cadastrarItem = (i, q) => formItem(null, {
      prefill: { descricao: q, unidade: itens[i].unidade },
    ...
    const catSel = el.querySelector('[name=categoria]');
    const catAtual = () => catSel.value;
    const somaServ = () => servs.reduce((t, x) => t + (Number(x.valorUnit) || 0), 0);
    const atualizarTotal = () => {
      const cat = catAtual();
      const frete = Math.max(0, Number(freteIn.value) || 0);
      const prod = S.temProdutos(cat) ? S.calcTotal(itens) : 0, serv = S.temServicos(cat) ? Math.round(somaServ() * 100) / 100 : 0;
    ...
    // --- serviços (categoria Prestação de Serviços ou Ambos) ---
    const pintarServs = () => {
      servBody.innerHTML = servs.map((x, i) => `<tr data-s="${i}">
        <td><input data-sk="descricao" value="${esc(x.descricao)}" placeholder="Ex.: mão de obra de instalação" required></td>
        <td><input data-sk="valorUnit" type="number" min="0.01" step="0.01" value="${esc(x.valorUnit)}" placeholder="0,00" required></td>
        <td><input data-sk="prazo" value="${esc(x.prazo)}" placeholder="Ex.: 5 dias úteis"></td>
        <td class="r num sub">${brl(Number(x.valorUnit) || 0)}</td>
        <td><button type="button" class="btn btn-danger btn-sm" data-rms="${i}" title="Remover" ${servs.length === 1 ? 'disabled' : ''}>${icon('trash', 16)}</button></td>
      </tr>`).join('');
      atualizarTotal();
    };
    servBody.addEventListener('input', e => { const tr = e.target.closest('tr'); if (!tr || !e.target.dataset.sk) return; servs[+tr.dataset.s][e.target.dataset.sk] = e.target.value; atualizarTotal(); });
    servBody.addEventListener('click', e => { const b = e.target.closest('[data-rms]'); if (b) { servs.splice(+b.dataset.rms, 1); pintarServs(); } });
    el.querySelector('#addServ').onclick = () => { servs.push(novoServ()); pintarServs(); servBody.querySelector('tr:last-child input').focus(); };

    // --- categoria: Produtos / Serviços / Ambos ---
    const cardProd = el.querySelector('#cardProd'), cardServ = el.querySelector('#cardServ');
    ...
    // --- modalidade: Manutenção de Veículos troca o centro de custo por placa ---
    const modSel = el.querySelector('[name=modalidade]'), ccSel = el.querySelector('#selCC'), boxVeic = el.querySelector('#boxVeic');
    const tmSel = el.querySelector('[name=tipoManutencao]'), boxOutro = el.querySelector('#boxOutro');
    const montarCC = () => {
      ...
      if (veic) {
        ...
        ccSel.innerHTML = '<option value="">Selecione…</option>' + opts(fl, atual) + addOpt;
        hint.textContent = '';
      }
      ccSel.dataset.ant = ccSel.value;
    };
    ...
    el.querySelector('#fNova').addEventListener('submit', e => {
      ...
      const cat = catAtual(), veic = S.ehVeiculo(f.modalidade.value);
      const linhas = (S.temProdutos(cat) ? itens.map(i => Object.assign({ tipo: 'produto' }, i)) : [])
        .concat(S.temServicos(cat) ? servs.map(x => ({ tipo: 'servico', descricao: x.descricao, valorUnit: x.valorUnit, prazo: x.prazo })) : []);
  ...
  function vLista(el, u) {
    const podeVerTodas = u.nivel >= 2;   // o servidor já entrega só o que o perfil pode ver
    const veTudo = nv(u) >= 3 || FIN(u);
    const sub = veTudo ? 'Todas as solicitações da empresa.' : nv(u) === 2 ? `Solicitações do departamento ${esc(u.departamento || '(sem departamento)')}.` : 'Suas solicitações de compra.';
    el.innerHTML = `
      <div class="page-head"><div><h1>Solicitações</h1><p>${sub}</p></div>
      ...
      <div class="card">
        <div class="filters">
          ...
          <select id="fCC"><option value="">Todas as modalidades</option>${opts(S.listas().modalidade)}</select>
          ${veTudo ? `<select id="fDep"><option value="">Todos os departamentos</option>${opts(S.listas().departamento || [])}</select>` : ''}
    ...
    const pintar = () => {
      ...
      const dep = el.querySelector('#fDep') ? el.querySelector('#fDep').value : '';
      ...
      if (cc) l = l.filter(r => S.norm(r.modalidade) === S.norm(cc));
      if (dep) l = l.filter(r => S.norm(r.departamento) === S.norm(dep));
      if (q) l = l.filter(r => [r.numero, r.numeroPdf, r.departamento, r.fornecedor, r.fornecedorFinal, r.solicitanteNome, r.centroCusto, r.modalidade, r.tipoManutencao, ...r.itens.map(i => i.descricao)].join(' ').toLowerCase().includes(q));
  ...
  function vAprovacoes(el, u) {
    ...
    const acima = u.nivel >= 2 ? todas.filter(r => !paraMim.includes(r) && r.solicitanteId !== u.id) : [];
    ...
    el.innerHTML = `
      ...
      ${u.nivel >= 2 ? `<div class="card"><div class="card-head"><h2>${separarDep ? `Do meu departamento — ${esc(u.departamento)} (${doDep.length})` : `Para você autorizar (${paraMim.length})`}</h2><span class="small muted">alçada até ${brl(S.limiteDoUsuario(u))}${u.nivel >= 3 ? ' · inclui as enviadas sem os orçamentos mínimos' : ''}</span></div>
  ...
  function abrirDetalhe(id, foco) {
    ...
    abrirGaveta(`
      ...
      <div class="table-wrap"><table><thead><tr><th>Descrição</th><th>Prazo</th><th class="r">Valor</th><th class="r">Subtotal</th></tr></thead>
        <tbody>${servs.map(i => `<tr><td>${esc(i.descricao)}</td><td>${esc(i.prazo || '—')}</td><td class="r num">${brl(i.valorUnit)}</td><td class="r num">${brl(i.valorUnit)}</td></tr>`).join('')}</tbody>
        <tfoot>${pe}</tfoot></table></div>` : '');
      })()}
  ...
  function vDashboard(el) {
    el.innerHTML = `
      ...
      <div class="filters">
        ...
        <select id="dCC"><option value="">Todas as modalidades</option>${opts(S.listas().modalidade)}</select>
      </div>
      <div id="dash"></div>`;
    ...
    const pintar = () => {
      ...
      const l = S.listRequests().filter(r => new Date(r.criadoEm).getTime() >= desde && (!fil || r.filial === fil) && (!cc || S.norm(r.modalidade) === S.norm(cc)));
      const soma = arr => arr.reduce((s, r) => s + Number(r.total), 0);
      ...
      const porCC = agrupar(aprov, r => r.modalidade || r.centroCusto);
      ...
      el.querySelector('#dash').innerHTML = `
        ...
        <div class="charts">
          ...
          <div class="card"><div class="card-head"><h2>Aprovado por modalidade</h2></div>${hbars(porCC, brlK)}</div>
  ...
  function vConfig(el, u) {
    ...
    el.innerHTML = `
      ...
      ${S.superAdmin() ? `<div class="card">
        ...
        <form id="fLim" class="grid g4" style="align-items:end">
          ${[1, 2, 3].map(n => `<div><label>${esc(S.nomeNivel(n))}${n === 3 ? ' — padrão' : ''} (R$)</label><input type="number" min="1" step="0.01" name="l${n}" value="${lim[n]}"></div>`).join('')}
        ...
        <h3 style="margin:18px 0 0">Teto do Gerente por departamento</h3>
        <p class="small muted" style="margin:4px 0 0">Cada gerente autoriza até o teto do seu departamento (em qualquer departamento). Em branco = usa o limite padrão do Gerente acima.</p>
        <div class="tetos">${(S.listas().departamento || []).map(d => { const g = users.filter(x => Number(x.nivel) === 3 && x.ativo && S.norm(x.departamento) === S.norm(d)).map(x => x.nome.split(' ')[0]).join(', ');
          const v = (lim.dep || {})[Object.keys(lim.dep || {}).find(k => S.norm(k) === S.norm(d))];
          return `<div><label>Gerente · ${esc(d)} (R$)</label><input type="number" min="1" step="0.01" data-teto="${esc(d)}" form="fLim" value="${v || ''}" placeholder="${brl(lim[3])}"><span class="small muted">${g ? esc(g) : 'sem gerente definido'}</span></div>`; }).join('')}</div>
        <p class="small muted" style="margin:10px 0 0">Acima do maior teto dos gerentes (${brl(S.maiorTetoGerente())}), o pedido fica aguardando <b>${esc(C.instanciaSuperior)}</b>. Novos limites valem para novas solicitações.</p>
      </div>` : ''}
      <div class="card">
    ...
    if (el.querySelector('#fLim')) el.querySelector('#fLim').addEventListener('submit', e => {
      ...
      const limitesDep = {}; el.querySelectorAll('[data-teto]').forEach(i => { limitesDep[i.dataset.teto] = i.value; });
      executar(f.querySelector('button'), () => S.act('saveLimites', { limites: { 1: f.l1.value, 2: f.l2.value, 3: f.l3.value }, limitesDep, minOrcamentos: f.minOrc.value }), 'Limites atualizados.');
    });
    ...
    const bPl = el.querySelector('#bPlacas');
    if (bPl) bPl.onclick = () => executar(bPl, () => S.act('atualizarPlacas', {}), j => `Placas atualizadas: ${(j.state.placas || []).filter(x => x.ativo).length} ativas.`, null, 'Lendo a planilha da frota…');
